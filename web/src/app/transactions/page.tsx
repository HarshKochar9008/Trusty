    ...
    if (activeTab === 0) {
      ...
      csvContent += "Transaction ID,Hash,Status,Confidence,Risk Level,Patient Name,Issuer,Date,Passed Checks,Total Checks,Timestamp\n" +
        filteredDocs.map((doc: HealthRecordTransaction) => 
          `${doc.transactionId},${doc.hash},${doc.status},${doc.aiVerification?.confidence || 'N/A'},${doc.aiVerification?.riskLevel || 'N/A'},${doc.patientInfo?.name || 'N/A'},${doc.patientInfo?.issuer || 'N/A'},${doc.patientInfo?.date || 'N/A'},${doc.aiVerification?.passedChecks || 0},${doc.aiVerification?.totalChecks || 0},${doc.timestamp}`
        ).join('\n');
    }
                  ...
                  {filterTransactions(messages, 'hedera').map((msg, idx) => (
                    <Card key={idx} sx={{ 
                      ...
                      <CardContent sx={{ p: 3 }}>
                        ...
                        <Button 
                          onClick={() => {
                            const hashscanUrl = `https://hashscan.io/testnet/transaction/${msg.sequenceNumber}`;
                        ...
                        <Card key={idx} sx={{ 
                          ...
                          <CardContent sx={{ p: 3 }}>
                            ...
                            <Accordion sx={{ 
                              ...
                              <AccordionDetails>
                                <Box sx={{ 
                                  ...
                                  {doc.aiVerification?.verificationMethods && Object.entries(doc.aiVerification.verificationMethods).map(([methodName, methodData]: [string, any], methodIdx: number) => (
                                    <Chip 
                                      key={methodIdx}
                            ...
                            <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
                              <Button 
                                onClick={() => {
                                  const hashscanUrl = `https://hashscan.io/testnet/transaction/${doc.sequenceNumber}`;
                                  window.open(hashscanUrl, '_blank');
                                }}
                                variant="outlined"
                                size="small"
